...
#End Region
    ...
    Private Sub BuildNoseCone()
        ...
        Try
            ...
            ' Create Shaft (Revolution/Revolve feature in CATIA V5)
            ' Shaft requires: sketch, axis_reference, angle
            Try
                ' Get Z axis for revolution
                Dim oZAxis As Object = oOriginElements.AxisZ
                Dim oZAxisRef As Object = oPart.CreateReferenceFromObject(oZAxis)

                ' Try Shaft with axis parameter
                Dim oShaft As Object = oSF.AddNewShaft(oConeSketch, oZAxisRef, 360.0)
                oShaft.Name = "Nose_Cone_Shaft"
                oPart.Update()
                Log("      [✓] Nose cone shaft created (360° around Z axis)")
                Log("   [✓] Nose cone complete - realistic conical shape!")
            Catch exShaft As Exception
                Log("      [WARN] Shaft with axis failed: " & exShaft.Message)
                ' Fallback: try without axis parameter (some CATIA versions)
                Try
                    Dim oShaft As Object = oSF.AddNewShaft(oConeSketch, 360.0)
                    oShaft.Name = "Nose_Cone_Shaft"
                    oPart.Update()
                    Log("      [✓] Nose cone shaft created (using default axis)")
                Catch exFallback As Exception
                    Log("      [INFO] Nose cone skipped - Shaft not available: " & exFallback.Message)
                End Try
            End Try
    ...
    Private Sub BuildTailCone()
        ...
        Try
            ...
            Try
                ...
                ' Try Shaft with axis parameter
                Dim oShaft As Object = oSF.AddNewShaft(oConeSketch, oZAxisRef, 360.0)
                oShaft.Name = "Tail_Cone_Shaft"
                oPart.Update()
                Log("      [✓] Tail cone shaft created (360° around Z axis)")
            ...
            Catch exShaft As Exception
                ...
                Try
                    ...
                    oShaft.Name = "Tail_Cone_Shaft"
                    oPart.Update()
                    Log("      [✓] Tail cone shaft created (using default axis)")
                Catch exFallback As Exception
                    Log("      [INFO] Tail cone skipped - Shaft not available: " & exFallback.Message)
                End Try
            End Try
