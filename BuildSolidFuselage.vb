...
#Region "Fields"
    ...
    Private ReadOnly AeroNoseLength As Double = 250.0
...
#End Region
    ...
    '
    '  Creates smooth cone by:
    '    1. Create base circle at X=0 (full radius R)
    '    2. Create tip point at X=-L
    '    3. Build cone with multiple continuous tapered sections
    ' =========================================================================
    Private Sub BuildAeroNoseEgg()
        Dim L As Double = AeroNoseLength          ' 250 mm -- nose length
        Dim R As Double = BulkheadWidth / 2.0     ' 100 mm -- base radius
        ...
        Try
            ...
            ' Create hybrid body for nose planes
            ...
            For section As Integer = 0 To numSections - 1
                ' Position along X axis: start from -L (tip) to 0 (base)
                ...
                Dim currentRadius As Double = R * Math.Sin(t * Math.PI / 2.0)  ' Sine curve = smoothest taper
                ...
                ' Create body and sketch with circle
                Dim oBody As Object = oBodies.Add()
    ...
    Private Sub BuildTailBoom()
        ...
        Dim boomRadius As Double = 45.0 / 2.0
        Dim boomLength As Double = 800.0
    ...
    Private Sub BuildNoseLandingGear()
        Try
            ...
            Dim strutLength As Double = 250.0
            Dim wheelRadius As Double = 25.0
            Dim tireThickness As Double = 8.0
            Dim strutRadius As Double = 6.0
            Dim bulkheadRadius As Double = BulkheadWidth / 2.0
            Dim strutStartZ As Double = -bulkheadRadius          ' top of strut  (-100mm)
            Dim bottomZ As Double = strutStartZ - strutLength    ' bottom of strut (-350mm)
            ...
            ' ---- Strut ----
            ' Sketch at BOTTOM of strut (bottomZ), extrude UPWARD by strutLength.
            ' This avoids IsDirectionOpposite which is not accessible via COM dispatch.
            ...
            oPart.Update()
            ' Positive length extrudes in +Z (upward) from bottomZ to strutStartZ
            ...
            ' Use YZ plane offset to X position to create XZ-like plane
    ...
    Private Sub BuildMainLandingGear()
        Try
            ...
            Dim strutLength As Double = 320.0
            Dim wheelRadius As Double = 30.0
            Dim tireThickness As Double = 10.0
            Dim strutRadius As Double = 8.0
            Dim wheelOffset As Double = -90.0
            Dim bulkheadRadius As Double = BulkheadWidth / 2.0
            Dim strutStartZ As Double = -bulkheadRadius + 70.0   ' top of strut (-30mm)
            Dim bottomZ As Double = strutStartZ - strutLength     ' bottom of strut (-350mm)
            ...
            ' ---- Wheels (on YZ plane offset to X = mainX, creating XZ-like orientation) ----
            ' LEFT WHEEL - YZ plane at X = mainX, left side
            ...
            ' RIGHT WHEEL - YZ plane at X = mainX, right side
